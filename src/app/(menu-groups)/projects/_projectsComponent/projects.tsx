...
export default function Projects({ currentCategory }: ProjectsProps) {
  ...
  // Loading State (Matches your ProjectPage style)
  if (loading) {
    return (
  ...
  // Empty State (If filter finds nothing)
  if (!loading && filteredProjects.length === 0) {
    return (
      <div className="flex justify-center items-center py-20">
        <div className="text-xl opacity-60 tracking-widest">
          NO PROJECTS FOUND.
        </div>
      </div>
    );
  ...
  return (
    <div className="p-2 grid md:grid-cols-2 items-center text-primary-dark dark:text-primary-light overflow-hidden">
      {filteredProjects.map((project, index) => {
        ...
        return (
          <Link
            ...
            prefetch
            key={project._id}
            className="block group" // Added 'group' to enable interactions on child elements
            scroll={false}
          >
            <div
              className={`
                flex items-center transform transition-all duration-500 ease-out cursor-pointer
                px-2 py-2 my-4
                ...
                ${
                  isEven
                    ? "pr-5 ml-10 rounded-l-[2rem] flex-row-reverse text-right dark:border-r-0"
                    : "pl-5 mr-10 rounded-r-[2rem] dark:border-l-0"
                }
              `}
            >
              {/* Card Icon */}
              <div
                className={`
                  w-20 h-24 p-2 mx-4 bg-card-white
                  flex justify-center items-center rounded drop-shadow-lg
                  transition-transform duration-300 ease-out
                  /* Added group-hover rotation for tactile feel */
                  ${
                    isEven
                      ? "text-card-red rotate-5 group-hover:rotate-12"
                      : "text-card-black -rotate-5 group-hover:-rotate-12"
                  } 
                `}
              >
              ...
              <div className="px-4 flex flex-col justify-between">
                <h3 className="text-xl font-bold group-hover:underline decoration-4 underline-offset-4">
                  {project.name}
                </h3>
                ...
                <div
                  className={`py-1 flex flex-wrap gap-2 ${
                    isEven ? "justify-end" : ""
                  }`}
                >
                  {project.technologies.map((tech) => (
                    <span
                      key={tech}
                      className="px-3 py-1 text-sm border-[3px] border-current rounded-full"
                    >
                      {tech}
                    </span>
                  ))}
